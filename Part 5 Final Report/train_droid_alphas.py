"""
    ...
    @NOTE
        - This file is used to train a Cross Entropy model as well as a CYBORG model
          with a variety of different parameters such as learning rates, epochs,
          batch sizes, etc.
"""
import os
import sys
...
"""
 ...
 ╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝     ╚═╝     ╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝╚═╝╚═╝  ╚═══╝ ╚═════╝ 
    ...
    For CYBORG
        - 0.5 for alpha_XENT
        - 0.5 for alpha_CYBORG

        - Going to test with a different split
            - 0.6 CYBROG
            - 0.4 XENT
"""
...
parser.add_argument('-csvPath', required=False, default= r'C:\Users\byron\OneDrive\Documents\Python Stuff\Computer Vision\Driving_Project\training.csv',type=str)
parser.add_argument('-datasetPath', required=False, default= r'C:\Users\byron\OneDrive\Documents\Python Stuff\Computer Vision\Driving_Project\\',type=str)
...
parser.add_argument('-heatmaps', required=False, default= r'C:\Users\byron\OneDrive\Documents\Python Stuff\Computer Vision\Driving_Project\Driving_Images\Heatmaps\\',type=str)
parser.add_argument('-alpha_vebm', required=False, default=0.0, type=float)
# parser.add_argument('-alpha_pcyborg', required=False, default=0.0, type=float)
parser.add_argument('-alpha_cyborg', required=False, default=0.6,type=float)
...
parser.add_argument('-alpha_xent', required=False, default=0.4,type=float)
# parser.add_argument('-alpha_xent', required=False, default=1.0,type=float)
parser.add_argument('-sgld_lr', required=False, default=1.0, type=float)
parser.add_argument("-sgld_std", type=float, default=1e-2)
parser.add_argument('-network', default= 'densenet',type=str)
parser.add_argument('-nClasses', default= 2,type=int)
parser.add_argument('-log',action='store_true')
## Test with keeprate below 1, maybe 0.5
# parser.add_argument('-keeprate', required=False, default=1.0,type=float)
parser.add_argument('-keeprate', required=False, default=0.5,type=float)
...
parser.add_argument('-mapSize', required=False, default=7, type=int)
...
## Densenet
    ...
    im_size = 224
    map_size = args.mapSize #The map size is 7 for densenet last layer
...
class_assgn = {'Not_Dangerous':0,'Dangerous':1}

"""
    Call to the dataset_loader.py file to load our testing, training, and validation sets
"""
...
"""
    Description of hyperparamters such as:
        - Loss Rate
        - Optimizer : SGD
...
log = {'iterations':[], 'epoch':[], 'validation':[], 'train_acc':[], 'val_acc':[], 'entropy':[],
        ...
        'loss_b4_back':[], 'interm_loss':[],'pcyborg_comp':[]}
...
"""
    Impending Changes -> [Completed]
        - in the dataset loader, it creates keep, list of true and false
            - it's random, and we need to modify it to be false for images without heatmaps
        - In the main code, need to set it to always use MSE loss reduction = mean instead of what it is currently
            - Currently it only use MSE loss if it has a keep loss less than 1
        - Try weighing CYBORG more and Cross-Entropy less
"""
# loop
for epoch in range(epoch_start, epoch_start + args.nEpochs):
    ...
    for phase in ['train', 'test']:
            ...
            for batch_idx, (data, cls, imageName, hmap, keep) in enumerate(tqdm(dataloader[phase])):
                ...
                if phase == 'train':
                    if args.network == "densenet":
                        ...
                        # parameters are the coefficients of the linear combinations of the maps for each class Not_Dangerous/Dangerous
                    ...
                    for ids,bd in enumerate(beforeDot):
                        ...
                        # select the weight set from the Not_Dangerous/Dangerous array per image in the batch
                    ...
                    if args.alpha_cyborg != 0:
                        ...
                        if not torch.isnan(cyborg_comp):
                            # print(cyborg_comp)
                    ...
                    if args.alpha_droid != 0:
                        if args.keeprate<1:
                            index = torch.tensor(np.where(keep)[0])
                            if len(index)>0:
                                index = index.to('cuda')
                                kentrops = torch.index_select(entrops,0,index)
                                droid_comp = args.alpha_droid * torch.mean(torch.log(kentrops + 1.))
                            else:
                                droid_comp = torch.tensor([0])
                        else:
                    ...
                    if args.alpha_vebm>0:
                        model.eval()
                        x_k = torch.autograd.Variable(data, requires_grad=True)
                        for k in range(1): #args.n_steps):
                            sgld_out = torch.gather(outputs.logsumexp(1), 1, cls[:, None])
                            f_prime = torch.autograd.grad(sgld_out.sum(), [x_k], retain_graph=True)[0]
                            x_k.data += args.sgld_lr * f_prime + args.sgld_std * t.randn_like(x_k)
                        model.train()
                        x_q = x_k.detach()
                        fp_all = outputs.logsumexp(1)
                        fq_all = model(x_q)
                        fp = fp_all.mean()
                        fq = fq_all.mean()
                        l_p_x = -(fp - fq)
                        hmap_loss += args.alpha_vebm * l_p_x

                    if args.alpha_pcyborg>0:
                        houts = model(hmap)
                        pred = torch.max(houts,dim=1)[1]
                        if args.network == "densenet":
                            features = model.features(hmap)
                            params = list(model.classifier.parameters())[0]
                        bz, nc, h, w = features.shape
                        beforeDot =  features.reshape((bz, nc, h*w))
                        hcams = []
                        ...
                        for ids,bd in enumerate(beforeDot):
                            weight = params[pred[ids]]
                            hcam = torch.matmul(weight, bd)
                            hcam_img = hcam.reshape(h, w)
                            hcam_img = hcam_img - torch.min(hcam_img)
                            hcam_img = hcam_img / torch.max(hcam_img)
                            hcams.append(hcam_img)
                        hcams = torch.stack(hcams)

                        pcyborg_comp = args.alpha_pcyborg * criterion_hmap(cams, hcams)
                        hmap_loss += pcyborg_comp
                        log['pcyborg_comp'].append(pcyborg_comp.item())
                ...
                if phase == 'train':
                    ...
                    if torch.is_tensor(hmap_loss):
                        hmap_loss = hmap_loss.to(device)
                    if args.log:
        ...
        if phase == 'train':
            ...
            if (accuracy >= bestAccuracy):
                ...
                bestEpoch = epoch
                save_best_model = os.path.join(log_path,'final_model_CYBORG.pth')
                states = {
                    'epoch': epoch + 1,
    ...
    with open(os.path.join(log_path,'model_log.json'), 'w') as out:
        json.dump(log, out)
    torch.save(states, os.path.join(log_path,f'current_CYBORG_model_100_epochs.pth'))
